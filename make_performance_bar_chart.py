...
def make_plot(df, out_filepath: str, palette: str = "dark", plot_type: str = "finetune") -> None: # from this example: https://seaborn.pydata.org/examples/grouped_barplot.html
    if plot_type == "finetune":
        ...
        order = ["GPT-2", "GPT-neo 1.3B", "GPT-3 Ada", "GPT-3 Davinci"]
    ...
    ci="sd", palette=palette, height=6, order=order
    )
    g.despine(left=True)
...
if __name__ == "__main__":
    ...
    df_zero_shot_finetune = df.loc[((df["Method"] == 'Zero-shot') | (df["Method"] == "Fine-tuning")) | (df["Method"] == "Human")]
    df_prompting = df[df["Method"].str.contains("Prompt")]
